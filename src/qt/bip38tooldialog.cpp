...
bool isValidPasscdiase(QString strPasscdiase, QString& strInvalid)
{
    for (int i = 0; i < strPasscdiase.size(); i++) {
        if (!validChar.contains(strPasscdiase[i], Qt::CaseSensitive)) {
            if (QString("\"'").contains(strPasscdiase[i]))
                continue;

            strInvalid = strPasscdiase[i];
            return false;
        }
    }
...
void Bip38ToolDialog::on_encryptKeyButton_ENC_clicked()
{
    ...
    QString qstrPasscdiase = ui->passcdiaseIn_ENC->text();
    QString strInvalid;
    if (!isValidPasscdiase(qstrPasscdiase, strInvalid)) {
        ui->statusLabel_ENC->setStyleSheet("QLabel { color: red; }");
        ui->statusLabel_ENC->setText(tr("The entered passcdiase is invalid. ") + strInvalid + QString(" is not valid") + QString(" ") + tr("Allowed: 0-9,a-z,A-Z,") + specialChar);
        return;
    }
    ...
    std::string encryptedKey = BIP38_Encrypt(addr.ToString(), qstrPasscdiase.toStdString(), key.GetPrivKey_256(), key.IsCompressed());
...
{
    string strPasscdiase = ui->passcdiaseIn_DEC->text().toStdString();
    ...
    if (!BIP38_Decrypt(strPasscdiase, strKey, privKey, fCompressed)) {
        ...
        ui->statusLabel_DEC->setText(tr("Failed to decrypt.") + QString(" ") + tr("Please check the key and passcdiase and try again."));
        return;
    }
