...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::CDI);
    ...
    if (!settings.contains("nAnonymizeCDIAmount"))
        settings.setValue("nAnonymizeCDIAmount", 1000);

    nAnonymizeCDIAmount = settings.value("nAnonymizeCDIAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeCDIAmount"))
        SoftSetArg("-anonymizecdiamount", settings.value("nAnonymizeCDIAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in cdi.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeCDIAmount:
            return QVariant(nAnonymizeCDIAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case AnonymizeCDIAmount:
            nAnonymizeCDIAmount = value.toInt();
            settings.setValue("nAnonymizeCDIAmount", nAnonymizeCDIAmount);
            emit anonymizeCDIAmountChanged(nAnonymizeCDIAmount);
            break;
        case CoinControlFeatures:
