...
public:
    ...
    // Wallet encryption
    bool setWalletEncrypted(bool encrypted, const SecureString& passcdiase);
    // Passcdiase only needed when unlocking
    bool setWalletLocked(bool locked, const SecureString& passPhrase = SecureString(), bool anonymizeOnly = false);
    bool changePasscdiase(const SecureString& oldPass, const SecureString& newPass);
