                                                          ...
                                                          currentBalance(-1)
{
    ...
    // "Spending 999999 zCDI ought to be enough for anybody." - Bill Gates, 2017
    ui->zCDIpayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzCDISyncStatus->setText("(" + tr("out of sync") + ")");
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        ui->pushButtonMintzCDI->setEnabled(false);
        ui->pushButtonMintzCDI->setToolTip(tr("zCDI is currently disabled due to maintenance."));

        ui->pushButtonSpendzCDI->setEnabled(false);
        ui->pushButtonSpendzCDI->setToolTip(tr("zCDI is currently disabled due to maintenance."));
    }
}
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zCDIpayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzCDI_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zCDI is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zCDI...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zCDI in ") +
...
void PrivacyDialog::on_pushButtonSpendzCDI_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zCDI is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        ...
        // Wallet is unlocked now, sedn zCDI
        sendzCDI();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zCDI
    sendzCDI();
}
...
{
    ui->labelzCDISelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzCDI()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid CDI Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->zCDIpayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zCDI is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " CDI </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zCDI</b>";
    ...
    if (walletModel && walletModel->getAddressTableModel()) {
        // If zCDI was spent successfully update the addressbook with the label
    ...
    // Clear zcdi selector in case it was used
    ZPhrControlDialog::listSelectedMints.clear();
    ui->labelzCDISelected_int->setText(QString("0"));
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zCDI </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zCDI "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zCDI "));
    ui->labelzCDIAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));
}
...
{
    ui->labelzCDISyncStatus->setVisible(fShow);
}
