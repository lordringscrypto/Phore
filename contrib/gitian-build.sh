...
VERSION=
commit=false
url=https://github.com/cdiproject/cdi
proc=2
mem=2000
lxc=true
...
Run this script from the directory containing the cdi, gitian-builder, gitian.sigs, and cdi-detached-sigs.
...
-u|--url	Specify the URL of the repository. Default is https://github.com/cdiproject/cdi
...
then
	...
	then
	    ...
	    echo ""
	    ./bin/gbuild -j ${proc} -m ${mem} --commit cdi=${COMMIT} --url cdi=${url} ../cdi/contrib/gitian-descriptors/gitian-linux.yml
	    ./bin/gsign --signer $SIGNER --release ${VERSION}-linux --destination ../gitian.sigs/ ../cdi/contrib/gitian-descriptors/gitian-linux.yml
	    mv build/out/cdi-*.tar.gz build/out/src/cdi-*.tar.gz ../cdi-binaries/${VERSION}
	fi
	# Windows
	if [[ $windows = true ]]
	then
	    ...
	    echo ""
	    ./bin/gbuild -j ${proc} -m ${mem} --commit cdi=${COMMIT} --url cdi=${url} ../cdi/contrib/gitian-descriptors/gitian-win.yml
	    ...
	    mv build/out/cdi-*.zip build/out/cdi-*.exe ../cdi-binaries/${VERSION}
	fi
	# Mac OSX
	if [[ $osx = true ]]
	then
	    ...
	    echo ""
	    ./bin/gbuild -j ${proc} -m ${mem} --commit cdi=${COMMIT} --url cdi=${url} ../cdi/contrib/gitian-descriptors/gitian-osx.yml
	    ...
	    mv build/out/cdi-*.tar.gz build/out/cdi-*.dmg ../cdi-binaries/${VERSION}
	fi
	# AArch64
	if [[ $aarch64 = true ]]
	then
	    ...
	    echo ""
	    ./bin/gbuild -j ${proc} -m ${mem} --commit cdi=${COMMIT} --url cdi=${url} ../cdi/contrib/gitian-descriptors/gitian-aarch64.yml
	    ./bin/gsign --signer $SIGNER --release ${VERSION}-aarch64 --destination ../gitian.sigs/ ../cdi/contrib/gitian-descriptors/gitian-aarch64.yml
	    mv build/out/cdi-*.tar.gz build/out/src/cdi-*.tar.gz ../cdi-binaries/${VERSION}
	fi
	popd || exit
...
then
	...
	then
	    ...
	    echo ""
	    ./bin/gbuild -i --commit signature=${COMMIT} ../cdi/contrib/gitian-descriptors/gitian-win-signer.yml
	    ...
	    mv build/out/cdi-*win64-setup.exe ../cdi-binaries/${VERSION}
	    mv build/out/cdi-*win32-setup.exe ../cdi-binaries/${VERSION}
	fi
	# Sign Mac OSX
	if [[ $osx = true ]]
	then
	    ...
	    echo ""
	    ./bin/gbuild -i --commit signature=${COMMIT} ../cdi/contrib/gitian-descriptors/gitian-osx-signer.yml
	    ...
	    mv build/out/cdi-osx-signed.dmg ../cdi-binaries/${VERSION}/cdi-${VERSION}-osx.dmg
	fi
	popd || exit
